...
function placeXOrO(squareNumber) {          //This function is for placing an X or an O in a square
    if(!selectedSquares.some(element => element.includes(squareNumber))) {   //Makes sure a square hasn't been selected, .some() will check each element of selectedSquares to see if it contains the square number clicked on.
        let select = document.getElementById(squareNumber);                   // This variable retrieves the html element id that was clicked
    
        if (activePlayer === 'X') {                                          //Check to see who's turn it is
            select.style.backgroundImage = 'url("images/newx.png")';            // if activePlayer is equal to X, the x.png is placed in HTML
        } else {                                                            // Active player ay only be 'X' or 'O' so, if not 'X' then must be 'O'
            select.style.backgroundImage = 'url("images/newo.png")';            //If activePlayer is equal to 'O', the o.png is placed in HTML
        }
        // Here squareNumber and activePlayer concatenated together and added to array//
        ...
        if(activePlayer === 'O') {
            ...
            disableClick();
            //This function waits 1 second before placing the image and then enabling click
    ...
    function computersTurn() {
        ...
        let pickASquare;
        //This condiation allows our while loop to keep trying if a square is selected already
        while(!success) {
...
function checkWinConditions() {
    ...
    //This condition checks for tie. If non of the above conditions register and 9 squares are selected, the code executes
    ...
    function arrayIncludes(squareA, squareB, squareC)   {
        ...
        // retured and our else if condition eecutes the drawWinLine function
